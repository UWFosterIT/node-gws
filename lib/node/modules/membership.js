...
class Membership extends Service {
  ...
  get(opt) {
    ...
    return this._get(`group/${opt.id}/${membership}`).then(result => {
  ...
  add(opt) {
    ...
    return this._put(`group/${opt.id}/member/${opt.netid}`).then(result => {
      return result;
    });
  }
  ...
  del(opt) {
    ...
    return this._del(`group/${opt.id}/member/${opt.netid}`).then(result => {
      return result;
    });
  }
}
